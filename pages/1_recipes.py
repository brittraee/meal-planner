...
# --- Add Recipe dialog ---
@st.dialog("Add Recipe", width="large")
def _add_recipe_dialog():
    """URL import or manual entry."""
    ...
    with tab_url:
        ...
        if "scraped_recipe" in st.session_state:
            ...
            data["protein"] = st.selectbox(
                "Protein", ["unknown", *proteins], index=0, key="url_protein",
            )
...
_link_col, _add_col, _ = st.columns([2, 2, 4])
with _link_col:
    st.page_link(
        "pages/2_planner.py",
...
with _add_col:
    if st.button("Add Recipe", icon=":material/add_circle:", type="tertiary"):
        _add_recipe_dialog()
